...
void MLXPlainTextEdit::appendColoredText( const QString &text )
{
	...
	QStringList segments = text.split( QRegExp( "(?=\\^\\d)" ), QString::SkipEmptyParts );
...
MLXMainWindow::MLXMainWindow()
{
	...
	connect( SearchBar, SIGNAL( QLineEdit::textChanged() ), this, SLOT( OnSearchTextChanged( const QString &text ) ) );
	...
	WholeWordCheck = new QCheckBox( "Whole Word", this );
...
void MLXMainWindow::UpdateTheme()
{
	if( mTheme == "Default" )
	{
		...
		mOutputWidget->DefaultOutputColor = QColor( "#242424" );
	}
...
void MLXMainWindow::OnHelpAbout()
{
	QMessageBox::about(
		this,
		"About ModLauncherX",
		QString( "ModLauncherX is a fork of the original Mod Launcher, with added features" ) +
		QString( " and quality of life changes.\n\nIf you want to request a change/feature, " ) +
...
void MLXMainWindow::OnOpenSZCFile()
{
	...
	else
	{
		QString ModName = Item->parent()->text( 0 );
		QString ZoneName = Item->text( 0 );
		ShellExecute( NULL, "open", ( QString( "\"%1/mods/%2/sound/zoneconfig/%3.zone\"" ).arg( mGamePath, ModName, ZoneName ) ).toLatin1().constData(), "", NULL, SW_SHOWDEFAULT );
	}
...
void MLXMainWindow::OnSaveOutputLog() const
{
	...
	QFile log(QString{ mToolsPath + "logs/modlog_%1.txt" }.arg(dateStr.c_str()));
